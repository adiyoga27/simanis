...
class _LzPupupWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (itemWidth >= Get.width) {
        dx += itemWidth;
      }

      // prevent dx from going out of screen
      if (dx < 0) dx = (0 + offset.dx);
      if ((dx + ddWidth + offset.dx) > Get.width) {
        dx = (Get.width - ddWidth - (offset.dx));
      }

      // prevent dy from going out of screen
      if (dy + ddHeight > Get.height) {
      ...
      double caretPosMax = (Get.width - (30 + offset.dx));
      ...
      double cx = itemWidth >= Get.width ? caretPosMax : caretXPos;
